...
public class SolicitudRepository : ISolicitudRepository
{
    ...
    public async Task<PagedResult<SolicitudApoyo>> ListarAsync(FiltroSolicitudes filtro, CancellationToken ct = default)
    {
        ...
        var items = await query.Skip((filtro.PageNumber - 1) * filtro.PageSize).Take(filtro.PageSize).ToListAsync(ct);

        return new PagedResult<SolicitudApoyo>(items, total, filtro.PageNumber, filtro.PageSize);
    }
